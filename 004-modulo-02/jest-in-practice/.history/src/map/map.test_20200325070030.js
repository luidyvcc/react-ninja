...
it('map(1) should throw a TypeError with message "The first parameter must be an array"', () => {
  ...
  expect(error).to.be.equal('The first parameter must be an array')
})
...
it('map([1, 2], "function") should throw a TypeError with message "func is not a function"', () => {
  let error
  try {
    map([1, 2], "function")
  } catch (e) {
  ...
  expect(error).to.be.equal('func is not a function')
})
